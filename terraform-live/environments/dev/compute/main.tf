...
module "compute" {
  source = "git::https://gitlab.com/demo-org/terraform-modules.git//compute?ref=v2.2.0"
  ...
  # Real ServiceAccount from the common-web-service chart release (see
  # expenseapp/values-backend.yaml's serviceAccount.annotations) -- v1.0.0 of
  # this module hardcoded "expense"/"expense-backend" (the old expenseinfra
  # project's naming), which silently didn't match this environment's real
  # namespace/ServiceAccount and broke IRSA auth (AssumeRoleWithWebIdentity
  # AccessDenied) until v2.0.0 made these required inputs instead.
  eks_expense_backend_sa_namespace = "team-payments"
  eks_expense_backend_sa_name      = "team-payments-backend-common-web-service"
}
